<?php
  /**
   *  Plugin Name: Romanian TV Online Widget
   *  Plugin URI: http://www.tvlive.im
   *  Description: RO: Cu ajutorul acestui plugin puteti afisa intr-un widget posturile dumneavoastra preferate. Acest plugin va permite sa alegeti dumneavoastra posturile tv pe care le afisati pe site-ul sau blogul dumneavoastra. EN : With this plugin you can display a widget in your favorite tv stations. This plugin allows you to choose the TV stations that you want to  display on your site or blog.
    ...
    if(get_option('tw_post_bifate') == TRUE){
      ...
      foreach($items as $item){
        ...
        foreach($item->data['child']['']['post'] as $post){
          ...
          if($i<=$max) echo '<li><a href="'.TV_API_PROVIDER.'post-'.$idpost.'.html" onclick="javascript: window.open(\''.TV_API_PROVIDER.'post-'.$idpost.'.html\', \'\', \'width='.TV_API_WIDTH.', height='.TV_API_HEIGHT.'\'); return false;" target="_blank" rel="nofollow" title="TvOnline '.$numepost.'">'.$numepost.'</a></li>';
          $i++;
        }
        echo '</ul></li>';
      }
      echo '<a  href="'.TV_API_PROVIDER.'" title="Tv Online" rel="nofollow" target="_blank">by TvLive.im</a>';
      echo '</ul>'; 
    } else {
